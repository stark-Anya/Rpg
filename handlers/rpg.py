...
async def expire_weapons(user_id, group_id, data, context):
    """Check and expire old weapons, notify user."""
        ...
        for name in expired:
            try:
                await context.bot.send_message(
                    chat_id=user_id,
                    text=f"""⏰ <b>Weapon Expired!</b>
🗡️ <b>{name}</b> has expired and been removed from your inventory.""",
                    parse_mode="HTML"
                )
            except Exception:
                pass
...
async def kill(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not tid:
        await update.message.reply_text("❌ Reply to a user to kill."); return
    if tid == killer.id:
    ...
    if kdata["status"] == "dead":
        await update.message.reply_text("💀 You are dead! Use /revive."); return

    vdata = await get_user(tid, gid, tname)
    if vdata["status"] == "dead":
        await update.message.reply_text(f"💀 <b>{tname}</b> is already dead!", parse_mode="HTML"); return
    if is_protected(vdata):
        await update.message.reply_text(f"🛡️ <b>{tname}</b> is protected!", parse_mode="HTML"); return

    # Expire weapons first
    kinv = await expire_weapons(killer.id, gid, kdata, context)

    # Get killer's best weapon bonus
    loot_bonus = 0.0
    ...
    # Victim's flex items go to killer
    ...
    for key, val in vinv.items():
        if key in FLEX_ITEMS:
            ...
            kinv2[key] = val  # transfer weapon too

    # Wanted bounty
    from database import db
    wanted_col = db["wanted"]
    today = int(time.time() // 86400)
    wanted_rec = await wanted_col.find_one({"user_id": tid, "group_id": gid, "day": today, "kills": {"$gte": WANTED_KILLS_THRESHOLD}})
    ...
    if rec:
        ...
        nk = 1
        await wanted_col.insert_one({"user_id": killer.id, "group_id": gid, "day": today, "username": killer.first_name, "kills": 1})

    await update_user(killer.id, gid, {"balance": new_kbal, "kills": kdata["kills"]+1, "inventory": kinv2})
    await update_user(tid, gid, {"balance": 0, "bank_balance": vdata["bank_balance"] - bank_loot, "status": "dead", "deaths": vdata["deaths"]+1, "inventory": {}})

    weapon_line = f"\n🗡️ <b>Weapon Bonus:</b> +{fmt(bonus_loot)} ({int(loot_bonus*100)}% via {best_item['name']})" if loot_bonus > 0 else ""
    bounty_line = f"\n🚨 <b>Bounty:</b> +{fmt(bounty)}" if bounty else ""
    wanted_line = f"\n\n🚨 <b>{killer.first_name} is now WANTED!</b>\n💰 Bounty: {fmt(WANTED_BOUNTY)} on their head!" if nk == WANTED_KILLS_THRESHOLD else ""

    caption = f"""⚔️ <b>{killer.first_name}</b> killed <b>{tname}</b>!
━━━━━━━━━━━━━━━
💰 <b>Wallet Loot:</b> {fmt(wallet_loot)}
🏦 <b>Bank Loot:</b> {fmt(bank_loot)}{weapon_line}{bounty_line}
━━━━━━━━━━━━━━━
💸 <b>Total:</b> {fmt(final_loot)}
👛 <b>{killer.first_name}'s Balance:</b> {fmt(new_kbal)}{wanted_line}"""
...
async def rob(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not tid or not amount:
        await update.message.reply_text("❌ Reply + <code>/rob [amount]</code>", parse_mode="HTML"); return
    if tid == u.id:
    ...
    if vdata["status"] == "dead":
        await update.message.reply_text(f"💀 {tname} is dead!", parse_mode="HTML"); return
    if is_protected(vdata):
        await update.message.reply_text(f"🛡️ {tname} is protected!", parse_mode="HTML"); return
    if vdata["balance"] < amount:
        await update.message.reply_text(f"❌ {tname} only has {fmt(vdata['balance'])}!", parse_mode="HTML"); return
...
💰 <b>Stolen:</b> {fmt(amount)}
👛 <b>Your Balance:</b> {fmt(udata['balance']+amount)}""")
...
async def protect(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if is_protected(data):
        ...
        await update.message.reply_text(f"🛡️ Already protected! <b>{rem//3600}h {(rem%3600)//60}m</b> left.", parse_mode="HTML")
        return

    # Parse duration: /protect 1d / 2d / 3d
    ...
    if arg not in costs:
        await update.message.reply_text(
            f"❌ Usage: <code>/protect 1d</code> / <code>/protect 2d</code> / <code>/protect 3d</code>\n"
            f"💰 <b>1d:</b> {fmt(PROTECT_COST_1D)} | <b>2d:</b> {fmt(PROTECT_COST_2D)} | <b>3d:</b> {fmt(PROTECT_COST_3D)}",
            parse_mode="HTML"); return

    cost = costs[arg]
    if data["balance"] < PROTECT_MIN_BALANCE:
        await update.message.reply_text(f"❌ Need at least {fmt(PROTECT_MIN_BALANCE)} in wallet.", parse_mode="HTML"); return
    if data["balance"] < cost:
        await update.message.reply_text(f"❌ Need {fmt(cost)} for {arg} protection.", parse_mode="HTML"); return
    ...
    await send_with_image(update, gid, IMG_PROTECT,
        f"""🛡️ <b>Protection Active!</b>
━━━━━━━━━━━━━━━
⏰ <b>Duration:</b> {arg}
💸 <b>Cost:</b> {fmt(cost)}
👛 <b>Balance:</b> {fmt(data['balance']-cost)}""")
...
async def hp(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if update.message.reply_to_message:
        ...
        data = await get_user(t.id, gid, t.first_name); name = t.first_name
    else:
        u = update.effective_user
        data = await get_user(u.id, gid, u.first_name); name = u.first_name
...
async def profile(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if update.message.reply_to_message:
        t = update.message.reply_to_message.from_user
        data = await get_user(t.id, gid, t.first_name); name = t.first_name; uid = t.id
    else:
        u = update.effective_user
        data = await get_user(u.id, gid, u.first_name); name = u.first_name; uid = u.id
    ...
    if is_protected(data):
        rem = int(data["protected_until"] - time.time())
        prot_str = f"🛡️ {rem//3600}h {(rem%3600)//60}m left"
...
━━━━━━━━━━━━━━━
{'🟢 Alive' if data['status']=='alive' else '💀 Dead'}
💰 <b>Wallet:</b> {fmt(data['balance'])}
🏦 <b>Bank:</b> {fmt(data['bank_balance'])}
📊 <b>Net Worth:</b> {fmt(nw)}
━━━━━━━━━━━━━━━
⚔️ <b>Kills:</b> {data['kills']} | 💀 <b>Deaths:</b> {data['deaths']}
🏆 <b>Wars:</b> {ws['wins']}W / {ws['losses']}L{stag}
━━━━━━━━━━━━━━━
🗡️ <b>Weapon:</b> {best_w}
🛡️ <b>Protection:</b> {prot_str}
💎 <b>Flex:</b> {flex_str}
...
async def topkill(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not users:
        await update.message.reply_text("No killers yet!"); return
    medals = ["🥇","🥈","🥉"]
    lines = "\n".join(f"{medals[i] if i<3 else f'{i+1}.'} <b>{u['username']}</b> — {u['kills']} kills" for i,u in enumerate(users))
    await update.message.reply_text(f"☠️ <b>Top Killers</b>\n━━━━━━━━━━━━━━━\n{lines}", parse_mode="HTML")
...
async def ranking(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    medals = ["🥇","🥈","🥉"]
    lines = "\n".join(f"{medals[i] if i<3 else f'{i+1}.'} <b>{u['username']}</b> | ⚔️{u['kills']} | {fmt(u['balance'])}" for i,u in enumerate(sorted_u))
    await update.message.reply_text(f"🏆 <b>Leaderboard</b>\n━━━━━━━━━━━━━━━\n{lines}", parse_mode="HTML")
