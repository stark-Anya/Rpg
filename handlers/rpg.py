...
async def kill(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    victim_loot = base_loot + bonus_loot

    # Extra kill reward (always given, regardless of victim balance)
    kill_reward = random.randint(KILL_REWARD_MIN, KILL_REWARD_MAX)

    # Transfer victim flex items & weapons to killer
    ...
    total_earned = victim_loot + kill_reward + bounty
    new_kbal     = kdata["balance"] + total_earned
    ...
    await update_user(killer.id, gid, {
        "balance": new_kbal,
        "kills": kdata["kills"] + 1,
        "inventory": kinv2
    })
    ...
    bounty_line = f"\n🚨 Bounty Collected: +{fmt(bounty)}" if bounty else ""
...
🏦 Bank loot: {fmt(bank_loot)}{weapon_line}
🎯 Kill reward: +{fmt(kill_reward)}{bounty_line}
━━━━━━━━━━━━━━━
💸 Total earned: {fmt(total_earned)}
👛 Your balance: {fmt(new_kbal)}{wanted_line}"""
