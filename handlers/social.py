...
async def propose(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if proposer["balance"] < tax:
        await update.message.reply_text(
            f"❌ Not enough coins!\nProposal tax: {format_coins(tax)} (5% of balance)",
            parse_mode="HTML"
        )
        return
    ...
    text = f"""💍 <b>{user.first_name}</b> is proposing to <b>{target_name}</b>!
━━━━━━━━━━━━━━━
💰 Proposal tax: {format_coins(tax)} (5%)

<b>{target_name}</b>, will you accept? 💕"""

    await send_with_image(
        context.bot, group_id, IMG_PROPOSE, text,
...
async def propose_button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("prop_accept_"):
        ...
        if not prop:
            await query.answer("💔 Proposal no longer valid!", show_alert=True)
            return
        ...
        text = f"""💍 <b>They're Married!</b>
━━━━━━━━━━━━━━━
🎉 <b>{prop['proposer_name']}</b> &amp; <b>{prop['target_name']}</b>
💰 Tax paid: {format_coins(prop['tax'])}

❤️ Congratulations to the happy couple!"""
        ...
        try:
            await query.edit_message_caption(
                caption=f"""💔 <b>{prop['target_name']}</b> declined the proposal!
😢 <b>{prop['proposer_name']}</b>'s heart is broken...
<blockquote>Sometimes love doesn't work out. Try again! 💪</blockquote>""",
                parse_mode="HTML",
                reply_markup=None
            )
        except Exception:
            await query.edit_message_text(
                text=f"""💔 <b>{prop['target_name']}</b> declined the proposal!
😢 <b>{prop['proposer_name']}</b>'s heart is broken...""",
                parse_mode="HTML",
                reply_markup=None
            )
...
Use /propose @user to find your match! 💍""",
    ...
    await update.message.reply_text(
        f"""💑 <b>{user.first_name}</b> is married to <b>{partner.get('username', 'Unknown')}</b>!
❤️ A beautiful couple!""",
        parse_mode="HTML"
    )
...
💰 You have: {format_coins(data['balance'])}""",
    ...
    text = f"""💔 <b>Divorce Finalized!</b>
━━━━━━━━━━━━━━━
<b>{user.first_name}</b> and <b>{partner_name}</b> are no longer together.
💸 Divorce cost: {format_coins(DIVORCE_COST)}
👛 Balance: {format_coins(new_balance)}

<blockquote>😢 It's over... but life goes on! 💪</blockquote>"""

    await send_with_image(context.bot, update.effective_chat.id, IMG_DIVORCE, text)
...
async def couple(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if len(users) < 2:
        await update.message.reply_text("❌ Not enough users for matchmaking!")
        return
